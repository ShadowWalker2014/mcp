...
function createMcpServer(stripeClient) {
...
function addStripeTools(server, stripe) {
        ...
        try {
            ...
            switch (action) {
                case 'create':
                    ...
                    const createParams = { name };
                    if (description)
                        createParams.description = description;
                    if (images)
                        createParams.images = images;
                    if (parsedMetadata)
                        createParams.metadata = parsedMetadata;
                ...
                case 'update':
                    ...
                    if (parsedMetadata)
                        updateParams.metadata = parsedMetadata;
                    if (active !== undefined)
        ...
        try {
            switch (action) {
                ...
                case 'update':
                    ...
                    if (enabled_events)
                        updateParams.enabled_events = enabled_events;
                    if (description !== undefined)
                        updateParams.description = description;
                    if (enabled !== undefined)
                        updateParams.disabled = !enabled;
    ...
    }, async ({ action, configuration_id, business_profile_headline, business_profile_privacy_policy_url, business_profile_terms_of_service_url, default_return_url, features_customer_update_allowed_updates, features_invoice_history_enabled = true, features_payment_method_update_enabled = true, features_subscription_cancel_enabled = true, features_subscription_pause_enabled = false, features_subscription_update_enabled = true }) => {
        try {
            switch (action) {
                case 'create':
                    const createParams = {
                    ...
                    if (business_profile_headline || business_profile_privacy_policy_url || business_profile_terms_of_service_url) {
                        ...
                        if (business_profile_headline)
                            createParams.business_profile.headline = business_profile_headline;
                        if (business_profile_privacy_policy_url)
                            createParams.business_profile.privacy_policy_url = business_profile_privacy_policy_url;
                        if (business_profile_terms_of_service_url)
                            createParams.business_profile.terms_of_service_url = business_profile_terms_of_service_url;
                    }
                    if (default_return_url)
                        createParams.default_return_url = default_return_url;
                ...
                case 'update':
                    ...
                    if (business_profile_headline || business_profile_privacy_policy_url || business_profile_terms_of_service_url) {
                        ...
                        if (business_profile_headline)
                            updateParams.business_profile.headline = business_profile_headline;
                        if (business_profile_privacy_policy_url)
                            updateParams.business_profile.privacy_policy_url = business_profile_privacy_policy_url;
                        if (business_profile_terms_of_service_url)
                            updateParams.business_profile.terms_of_service_url = business_profile_terms_of_service_url;
                    }
                    if (default_return_url)
                        updateParams.default_return_url = default_return_url;
    ...
    server.tool("stripe_query", {
        ...
        try {
            ...
            // Add filters
            if (filters.created)
                params.created = filters.created;
            if (filters.status)
            ...
            if (filters.payment_intent)
                params.payment_intent = filters.payment_intent;
            if (filters.amount)
                        ...
                        req.on('end', async () => {
                            try {
                                ...
                                if (!sessionId && isInitializeRequest(requestBody)) {
                                    ...
                                    const stripeApiKey = req.headers['x-stripe-api-key'] ||
                                        req.headers['stripe-api-key'] ||
